...
return function(library)
    ...
    -- Function to hide all ESP elements for a player
    ...
    -- Function to hide all ESP elements for all players
    local function HideAllESP()
        for player, _ in pairs(ESPObjects.Box2D) do
            HideESP(player)
        end
    end
    ...
    local espConnection = RunService.RenderStepped:Connect(function()
        ...
        for player, objects in pairs(ESPObjects.Box2D) do
            ...
            if GetFlag("esp_box") and GetFlag("name_type") == "2D" then
    ...
    -- Return the ESP module with control functions
    return {
        ...
        -- Function to update a specific ESP feature
        updateFeature = function(featureName, enabled)
            -- This function can be expanded to toggle specific ESP features
            -- For example: updateFeature("box", true) to enable only box ESP
            if featureName == "box" then
                library.flags.esp_box = enabled
            elseif featureName == "name" then
                library.flags.esp_name = enabled
            elseif featureName == "distance" then
                library.flags.esp_distance = enabled
            elseif featureName == "weapon" then
                library.flags.esp_weapon = enabled
            elseif featureName == "healthbar" then
                library.flags.esp_healthbar = enabled
            elseif featureName == "armor" then
                library.flags.esp_armor = enabled
            elseif featureName == "chams" then
